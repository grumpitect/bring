...
var resolveModulePath = function(modulePath){
    ...
    if(cachedModule){
        ...
        if(modulePath.search(/^\/[^\/]/) !== -1){ // project's root directory [exacly one slash]
            ...
            if(!resolvedModulePath){
                var normalizedModulePath    = path.normalize(modulePath); // convert slash to backslash or viseversa
                var resolvedModulePaths     = _.filter(cache.allModuleFilePaths, function(aModulePath){
                    ...
                    if(index > -1){
                        ...
                        // ensure that we are at the end of the path (/api/services/user.js -> services/user.js), also support not specifying the extension
                        // commenting the "if" line will enable us to load modules from folders with an "index" file
                        // maybe I should prioritize files over directories
                        // if(index + modulePathLength === pathLength || index + modulePathLength === pathLength - extensionLength)
                        return true;
                    }
...
var bring = function(modulePath){
    ...
    else if(resolvedModulePath.name){ // it is an error object, throw it away!
